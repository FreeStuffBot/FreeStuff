...
export default class DockerInterface {
  ...
  public static connect() {
    ...
    setInterval(async () => {
      ...
      const { status: s1 } = await axios.get(`http://${b.networkIp}/`)
      Logger.debug('GET / -> ' + s1)
      const { status: s2 } = await axios.get(`http://${b.networkIp}/umi/info`)
      Logger.debug('GET /umi/info -> ' + s2)
      const { status: s3 } = await axios.get(`http://${b.networkIp}:80/umi/info`)
